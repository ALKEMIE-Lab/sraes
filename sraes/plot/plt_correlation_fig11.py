...
def read_data_for_plt(fn, has_first_column=True):
    if has_first_column:
        ...
        star_num = 1
    ori_data = read_data(fn)
    column = ori_data.columns.values.tolist()[star_num:-5]
    data = ori_data.values
    train_data = np.array(data[:, star_num:-5], dtype=float)
...
def plt_fig1():
    ...
    index_label_font = {"fontsize": 8, 'weight': 'light', 'family': 'Times New Roman'}
    tick_font_size = 14
    tick_font_dict = {"fontsize": 8, 'family': 'Times New Roman'}
    ...
    csv_file = os.path.join(head_dir, r'1.20230201_clean.xlsx')
    ...
    csv_file = os.path.join(head_dir, r'2.normalized_data.xlsx')
    ...
    # plt.tight_layout()
    plt.subplots_adjust(left=0.08, bottom=0.18, top=0.93, right=0.98, wspace=1.2)
    # plt.show()
