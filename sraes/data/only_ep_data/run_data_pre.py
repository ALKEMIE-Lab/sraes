...
from sraes.data.tools import get_random_from_each_groupby_index, get_random_groupby_index
from sraes.data.tools import auto_get_corelated_group
import numpy as np
import pandas as pd
...
def random_data():
    ...
    __rm_point_columns(train_train_data).to_excel('3.train_%d_train.xlsx'%train_train_data.shape[0] , sheet_name=SHEET_NAME, index=False)
    __rm_point_columns(train_test_data).to_excel('3.train_%d_test.xlsx'%train_test_data.shape[0], sheet_name=SHEET_NAME, index=False)
    __rm_point_columns(valid_10data).to_excel('4.%d_for_check.xlsx'%valid_10data.shape[0], sheet_name=SHEET_NAME, index=False)
    __rm_point_columns(valid_30data).to_excel('4.%d_for_predict.xlsx'%valid_30data.shape[0], sheet_name=SHEET_NAME, index=False)
