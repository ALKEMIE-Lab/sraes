...
def get_random_from_each_groupby_index(data, column_index, ratio, to_data=False,
    ...
    if train_index is not None:
        ...
        train_index = []
        for jdd, (kk, ss) in enumerate(dd.groups.items()):
            _tmp_index = norepeat_randint(dd.groups[kk].tolist(), ratio=ratio)
            for nn in _tmp_index:
                train_index.append(dd.groups[kk][nn])
            # if jdd in _tmp_index:
            #     train_index.append(int(data.loc[dd.groups[kk][0]]['Index'] * 69 + 1))
            # exit()
    ...
    if test_index is not None:
        ...
        test_index = [nn for nn in range(data.shape[0]) if nn not in train_index]
