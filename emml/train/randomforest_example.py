...
def tree_view(rf, data):
    # 选择一个决策树，这里选择第一个树
    tree_to_visualize = rf.estimators_[0]
    
    # 可视化该决策树
    plt.figure(figsize=(20, 10))
    plot_tree(tree_to_visualize, feature_names=feature_names, class_names=data['target_names'], filled=True, rounded=True)
    plt.show()
    ...
    for idx, tree in enumerate(rf.estimators_):
        ...
        plot_tree(tree, feature_names=feature_names, class_names=data['target_names'], filled=True, rounded=True, ax=ax)
...
if __name__ == '__main__':
    rf, dd, fi = run()
    feature_names = dd['feature_names']
    # plot_feature_importances(fi, feature_names)
    tree_view(rf, dd)
